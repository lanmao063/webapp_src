...
执行 `src/main/resources/db/init.sql` 创建表结构，然后执行 `test_data.sql` 导入测试数据。

如果数据库已有旧版本，执行 `alter_add_courier_id.sql`。
...
| 用户名 | 角色 | 说明 |
|--------|------|------|
| zhangsan | REGULAR | 普通用户 |
| lisi | REGULAR | 普通用户 |
| wangwu | REGULAR | 普通用户 |
| courier1 | COURIER | 快递员 |
| courier2 | COURIER | 快递员 |
| manager1 | MANAGER | 管理员 |
